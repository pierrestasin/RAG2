"""
Vector Store Service - Version légère pour Render free tier
Utilise l'API OpenAI Embeddings au lieu de SentenceTransformer local
pour économiser ~700MB de RAM
"""
import chromadb
...
class VectorStore:
    def __init__(self):
        ...
        # Crée nouvelle collection
        self.collection = self.client.create_collection(
            name="documents",
            metadata={"description": "RAG documents collection - OpenAI Embeddings"}
        )

        # Utilise OpenAI API pour embeddings (économise ~700MB RAM vs local model)
        self.openai_client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
        print("✓ Using OpenAI API for embeddings (text-embedding-3-small)")

        # Réranker Cohere (optionnel)
    ...
    def _get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        response = self.openai_client.embeddings.create(
            model="text-embedding-3-small",  # 1536 dimensions, très rapide et pas cher
            input=texts
        )
    ...
    def add_document(self, filename: str, chunks: List[str], metadatas: List[Dict]):
        ...
        # Génère les embeddings via API OpenAI
        print(f"🔄 Generating embeddings for {len(chunks)} chunks (via OpenAI API)...")
    ...
    def _get_optimal_n_results(self, model_name: str, base_n_results: int = 10) -> int:
        """Détermine le nombre optimal de chunks selon le modèle"""
    ...
    def search(
        ...
        # Si réranking activé, récupère plus de chunks
        initial_n = optimal_n * 2 if (use_reranking and self.cohere_client) else optimal_n

        # Génère embedding de la question via API
